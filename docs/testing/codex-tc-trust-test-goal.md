# MISSION — Empirical Tool-Surface Test of Terminal Commander (for Codex)

You are **Codex** (OpenAI's agentic CLI). The `terminal-commander` MCP server is
...
4. A single report file exists at **`outputs/codex-tc-test-report.md`**
   (relative to the Terminal Commander repo root, i.e.
   `E:\project\terminal-commander\outputs\codex-tc-test-report.md`) containing
...
2. **Record TC version.** Run `terminal-commander --version`. Capture the exact
   ...
   couldn't).
3. **Record harness identity.** Note that the executor is **Codex** and which
...
## REQUIRED REPORT FORMAT (write to `outputs/codex-tc-test-report.md`)
...
- Date of run
- Executor = Codex; model/harness identity
- Platform/OS
...
When `outputs/codex-tc-test-report.md` exists with all six sections, every prompt
