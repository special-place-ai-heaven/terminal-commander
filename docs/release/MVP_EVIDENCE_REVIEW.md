...
4. **WSL pair (Out-of-Scope):** `packages/terminal-commander/lib/cli/pair_accept.js:runPairAccept` returns `pair_deferred` — safe refuse, not queued elsewhere. See BACKLOG WWS-B6. This is **Out-of-Scope**, not `Deferred` or `Superseded-by-TCNN`.
...
| <a id="tc14"></a>TC14 | Live | `rules/README.md` · `EVIDENCE_REPORT_RUNTIME.md` §TC42 | Seed packs + import. |
...
| WWS06 pair | Out-of-Scope | `packages/terminal-commander/lib/cli/pair_accept.js:runPairAccept` | WSL handshake deferred; safe refuse (BACKLOG WWS-B6). |
