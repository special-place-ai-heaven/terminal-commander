...
# TC16 - Job Manager And Command Exit Events

Use this file directly with `/goal`:

    /goal .agent/goals/terminal-commander-mvp/TC16-job-manager-and-command-exit-events.md
...
objective:
- Implement the job manager that tracks process lifecycle, job status, cancellation, exit codes, non-zero exit events, and command runtime metadata.

non_goals:
- Do not expose MCP tools yet.
- Do not implement PTY or interactive stdin.
- Do not permit privileged command execution beyond policy placeholders.

allowed_files_or_area:
- .agent/goals/terminal-commander-mvp/TC16-job-manager-and-command-exit-events.md
...
- Every started command must have a job_id and status.
- A non-zero exit must emit or make available a high-severity command_failed/process_exit_nonzero signal event.
- Job status must include started_at, optional ended_at, exit_code or signal, bytes/frames seen, events emitted, and last_output_at when available.
...
- Define JobConfig, JobStatus, JobExitInfo, and JobManager interfaces in the appropriate crate.
- Integrate ProcessProbeHandle lifecycle with JobManager state transitions.
- Implement cancellation or signal sending for spawned jobs if supported by the platform abstraction.
- Emit or record process exit events for non-zero exit codes and successful completion milestones according to configured rules.
- Add tests for success exit, non-zero exit, cancellation/termination if supported, and status query behavior.

acceptance_criteria:
- JobManager starts, tracks, and completes safe test commands.
- Non-zero exit creates a structured signal event or equivalent tested event output.
- Job status is queryable while running and after exit.
- Cancellation behavior is implemented or explicitly documented as deferred with tests avoiding false claims.
- No MCP tool or privileged execution path is added.
...
git diff --check
cargo fmt --check
cargo test -p terminal-commanderd job
cargo test -p terminal-commander-probes process_probe
```

## Task Prompt

Run TC16 only on branch `feature/terminal-commander-mvp`. Complete the objective above, stay inside the allowed files/areas, respect all forbidden files and invariants, verify the work, commit only verified changes, update this goal file's status fields, and report blockers instead of guessing.
...
Next goal:
- TC17 and TC17-realtime-bucket-waiter.md
