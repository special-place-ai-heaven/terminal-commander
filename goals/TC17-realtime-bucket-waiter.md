...
- If no matching event arrives before timeout, return heartbeat=true and unchanged or current cursor without raw output.
...
git diff --check
cargo fmt --check
cargo test -p terminal-commanderd bucket_wait
```
