...
- Select and document a PTY abstraction compatible with Linux/WSL or mark exact limitations.
...
git diff --check
cargo fmt --check
cargo test -p terminal-commander-probes pty
cargo test -p terminal-commander-sifters prompt
```
