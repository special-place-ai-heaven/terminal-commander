...
| `terminal-commander` | **CLI surface** for the WWS chain. WWS06 added `doctor`, `doctor wsl`, `setup cursor-wsl`, `pair create`, `pair accept <code>`. INSTALL01+ adds `setup daemon-autostart` and `doctor daemon`. Global install/bootstrap installs a **systemd user service** (when WSL systemd is on) or a **profile autostart hook** so `terminal-commanderd` starts when the distro starts or on first MCP bridge use. Run `terminal-commander --help` for the full panel. |
