...
test("win32 x64 is rejected as unsupported_platform", () => {
  const r = resolveBinary({
    platform: "win32",
    arch: "x64",
    binary: "terminal-commanderd",
  ...
  assert.match(msg, /unsupported platform win32-x64/);
});
...
test("formatResolveError message is single-line and bounded", () => {
  const r = resolveBinary({
    platform: "win32",
    arch: "arm",
    binary: "terminal-commanderd",
  ...
  const msg = formatResolveError(r, { platform: "win32", arch: "arm" });
