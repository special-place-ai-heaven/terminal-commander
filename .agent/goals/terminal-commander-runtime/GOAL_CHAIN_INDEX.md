...
| TC42b | Live Rule Rebind For Active Streams | Pending | TC42 | main | Make registry activation/deactivation reach future frames produced by already-running command/probe streams without restart and without exposing raw output. |
| TC42c | Scoped Rule Bindings For Buckets Jobs Probes | Pending | TC42b | main | Implement scoped registry rule bindings so activated rules can target a specific bucket, job, probe, or explicit global scope, with live rebind and no cross-scope signal leakage. |
| TC42d | Explicit Scope And Nextest Gate | Pending | TC42c | main | Require explicit scope on registry_activate/deactivate (no silent Global default); install + run cargo nextest as first-class verification. |
| TC43 | File Probe Search Watch And Bounded Read | Pending | TC42d | main | Expose bounded file intelligence through probes/tools so the LLM can ask for file lists, targeted search, line windows, and watched file changes without reading whole files. |
