...
Use this file directly with `/goal`:
    ...
    /goal .agent/goals/terminal-commander-mvp/TC10-keyword-and-regex-sifter-runtime.md
...
objective:
- Implement the first sifter runtime that evaluates keyword and regex rules against normalized frames and emits structured signal event drafts.

non_goals:
- Do not implement registry persistence or live daemon activation.
- Do not implement multiline, numeric condition, progress, stall, or correlation rules.
- Do not run shell commands or watch files.

allowed_files_or_area:
- .agent/goals/terminal-commander-mvp/TC10-keyword-and-regex-sifter-runtime.md
- crates/terminal-commander-sifters/Cargo.toml
...
- The sifter runtime consumes SourceFrame-like input and active RuleDefinition values.
- A match creates a structured `EventDraft` (TC06) with severity, kind, summary, captures, rule reference, and source pointer; final event IDs are assigned by the BucketManager at TC07 `append`.
- Regex patterns must be compiled safely and validation failures must be reported explicitly.
...
- Add terminal-commander-sifters dependencies required for safe regex matching and core model integration.
- Implement a SifterRuntime or equivalent evaluator for keyword and regex rules.
- Implement capture extraction for named regex groups where supported.
- Convert matches into SignalEvent-compatible event drafts or complete events using test IDs.
- Add fixture-based tests for generic error keyword, apt missing package regex, gcc missing header regex, and no-match cases.

acceptance_criteria:
- Keyword rules match case sensitivity according to their definition.
- Regex rules extract named captures and render summaries.
- Invalid regex rules are rejected or reported without panicking.
- Every emitted match includes a source pointer to the triggering frame.
- Tests prove non-matching noise lines do not emit signal events.
...
Run TC10 only on branch `feature/terminal-commander-mvp`. Complete the objective above, stay inside the allowed files/areas, respect all forbidden files and invariants, verify the work, commit only verified changes, update this goal file's status fields, and report blockers instead of guessing.
...
Next goal:
- TC11 and TC11-noise-suppression-dedupe-and-progress-basics.md
