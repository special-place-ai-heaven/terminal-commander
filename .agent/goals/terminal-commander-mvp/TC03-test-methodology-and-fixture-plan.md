...
- TESTING.md MUST enumerate the 7-step CI sequence (from docs/research/_R2-gamma-summary.md): cargo fmt --check, cargo clippy -D warnings, cargo-deny, cargo-hack --each-feature, cargo-hack --rust-version, cargo-nextest run plus doctest, cargo-machete.
...
- Add fixture category `tests/fixtures/probes/wsl-mountinfo/` containing sample 9P and native `/proc/self/mountinfo` records (used by TC18/TC20/TC25 to detect WSL `/mnt/c` and force PollWatcher).
