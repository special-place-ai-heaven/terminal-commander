...
# TC18 - File Probe Follow Create And Rotate

Use this file directly with `/goal`:

    /goal .agent/goals/terminal-commander-mvp/TC18-file-probe-follow-create-and-rotate.md
...
objective:
- Implement a file probe that can scan or follow files, handle creation after watch start, truncation, and rotation while emitting signal events and bounded context.

non_goals:
- Do not implement directory tree watch or artifact parsing.
- Do not expose unrestricted file read APIs.
- Do not bypass policy checks; use placeholders if policy is not implemented yet and clearly mark them.

allowed_files_or_area:
- .agent/goals/terminal-commander-mvp/TC18-file-probe-follow-create-and-rotate.md
...
- FileProbeConfig must specify path, mode, starting position, bucket, and active rules or rule set references.
- File probe must preserve source pointers with line and byte metadata where possible.
...
- Define FileProbeConfig, FileProbeMode, and file source descriptor types.
- Implement read-existing and follow-from-end modes with incremental frame emission.
- Handle file created after probe start, truncation, and inode/path replacement. Rotation/replacement detection via parent-directory watch on Linux + WSL2-native FS; via polling stat on 9P. macOS/Windows-native deferred (FSEvents 32-subpath limit + ReadDirectoryChangesW caveats).
- Feed new frames into the sifter runtime and event sink with context ring support.
- Add tests for existing file scan, append follow, create later, truncate, and rotation/replacement behavior if platform support allows.

acceptance_criteria:
- File probe emits signal events for appended matching lines.
- Existing file scan mode and follow-from-end mode behave differently and are tested.
- Missing file watch behavior is deterministic and documented.
- Context around a file match can be retrieved through source pointers in tests.
- No unrestricted full-file dump API is added.
- On Linux/WSL2, probe detects 9P mounts via `/proc/self/mountinfo` at probe construction and forces `PollWatcher`. Test fixture demonstrates a path under a 9P mount triggers polling-backend selection, NOT inotify. Cite microsoft/WSL#4739.
...
Run TC18 only on branch `feature/terminal-commander-mvp`. Complete the objective above, stay inside the allowed files/areas, respect all forbidden files and invariants, verify the work, commit only verified changes, update this goal file's status fields, and report blockers instead of guessing.
...
Next goal:
- TC19 and TC19-terminal-pty-probe-and-prompt-detection.md
