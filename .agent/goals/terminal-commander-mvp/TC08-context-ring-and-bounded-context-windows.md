...
# TC08 - Context Ring And Bounded Context Windows

Use this file directly with `/goal`:

    /goal .agent/goals/terminal-commander-mvp/TC08-context-ring-and-bounded-context-windows.md
...
objective:
- Implement the bounded context ring model that stores normalized frames and returns explicit before/after context windows around source pointers.

non_goals:
- Do not implement persistent context spool or compression.
- Do not read real terminal streams or files.
- Do not expose unbounded raw source access.

allowed_files_or_area:
- .agent/goals/terminal-commander-mvp/TC08-context-ring-and-bounded-context-windows.md
...
- Context windows must require explicit before and after limits.
- Frames must preserve source_id, frame_id, stream, timestamp, text, and optional line/byte metadata.
- Context lookup must return truncation metadata when requested context is unavailable.
...
- Define SourceFrame, ContextWindowRequest, ContextLine, and ContextWindowResponse types.
- Implement an in-memory bounded ContextRing per source or probe.
- Implement append_frame and window_around_pointer with before/after controls.
- Preserve pointer resolution semantics for frame_id and line-based access where available.
- Add tests for exact window, truncated before, truncated after, missing pointer, and long-line handling.

acceptance_criteria:
- ContextRing returns only the requested bounded window and marks truncation correctly.
- Context frames can be connected to SourcePointer from TC06.
- Long lines are bounded or explicitly marked according to the contract.
- Missing context returns a structured unavailable result, not a panic or fake context.
- Tests cover before/after behavior around a signal frame.
...
cargo nextest run -p terminal-commander-core -E 'test(context)'
```

## Task Prompt

Run TC08 only on branch `feature/terminal-commander-mvp`. Complete the objective above, stay inside the allowed files/areas, respect all forbidden files and invariants, verify the work, commit only verified changes, update this goal file's status fields, and report blockers instead of guessing.
...
Next goal:
- TC09 and TC09-rule-model-validation-and-templates.md
