...
# TC11 - Noise Suppression Dedupe And Progress Basics

Use this file directly with `/goal`:

    /goal .agent/goals/terminal-commander-mvp/TC11-noise-suppression-dedupe-and-progress-basics.md
...
objective:
- Add basic deduplication, suppression, repeated-event collapse, and progress-noise classification to the sifter runtime.

non_goals:
- Do not implement full correlation or advanced state machines.
- Do not change bucket persistence or MCP tools.
- Do not suppress high-severity unique events.

allowed_files_or_area:
- .agent/goals/terminal-commander-mvp/TC11-noise-suppression-dedupe-and-progress-basics.md
...
- Deduplication must preserve representative event, count, first_seen, and last_seen where practical.
- Suppression must be explicit and counted; it must not silently delete all evidence.
- Progress-like output should not flood buckets by default.
- Progress detection operates on post-ANSI-strip frames; carriage-return (CR) collapse is performed upstream in TC15 (process probe), not here.
...
- Define noise policy and suppression result types if not already defined.
- Implement dedupe keys for repeated identical rule matches within a configurable window.
- Implement repeated-event collapse summaries for low/medium repeated events.
- Implement simple progress-line detection for carriage-return progress, percentage-only updates, spinner-only updates, and repeated status frames.
- Add tests proving high-severity distinct events are not suppressed and repeated noise is counted.

acceptance_criteria:
- Repeated identical low/medium events collapse into a count-aware representation or suppress duplicate emissions with counters.
- Progress-like frames are classified as noise under strict policy.
- Suppression counters are available to bucket summaries or event processing outputs.
- High-severity events with different captures remain distinct.
- Tests cover repeated warning collapse and progress suppression.
...
Run TC11 only on branch `feature/terminal-commander-mvp`. Complete the objective above, stay inside the allowed files/areas, respect all forbidden files and invariants, verify the work, commit only verified changes, update this goal file's status fields, and report blockers instead of guessing.
...
Next goal:
- TC12 and TC12-persistent-event-store-and-bucket-cursors.md
