...
Use this file directly with `/goal`:
    ...
    /goal .agent/goals/terminal-commander-mvp/TC27-provider-harness-integration-examples.md
...
objective:
- Create provider-neutral MCP integration examples and harness notes for Claude Code, Codex CLI, and generic MCP clients without hardcoding secrets or machine-specific paths.

non_goals:
- Do not depend on provider-specific hooks for core correctness.
- Do not commit user-specific tokens, absolute private paths, or credentials.
- Do not claim integration is verified unless commands were actually run and evidence is recorded.

allowed_files_or_area:
- .agent/goals/terminal-commander-mvp/TC27-provider-harness-integration-examples.md
- docs/integrations/**
- examples/mcp/**
- examples/workflows/**
- README.md
...
- Core realtime behavior must remain based on daemon probes plus bucket_wait, not provider-specific hooks.
- Integration examples must show how to configure the MCP server and how an LLM should call tools in sequence.
- Examples must be parameterized and safe for local adaptation.
- Integration examples target MCP spec revision 2025-11-25 served by `rmcp = "=1.7.0"`. Example configs reference the stdio transport (rmcp default for TC23).
...
- Create docs/integrations/README.md explaining provider-neutral MCP setup assumptions.
- Create stdio-only MCP config examples. Document that local-socket transport between MCP server and daemon is internal-only and not exposed to providers.
- Create Claude Code and Codex CLI notes only to the extent supported by available evidence or clearly label as template/unverified. <<DECISION REQUIRED: Claude Code MCP config file path + JSON shape on Linux/WSL>> <<DECISION REQUIRED: Codex CLI MCP attachment syntax>>
- Create workflow examples for running tests, watching install output, creating a registry rule, activating it, and waiting on a bucket.
- Update README.md with a short integration pointer without overstating product maturity.

acceptance_criteria:
- Integration examples are free of secrets and machine-specific private paths.
- Docs explain that bucket_wait is the provider-neutral nudge fallback.
- Each example identifies whether it is verified, template-only, partial, or blocked.
- Each provider example records the provider version observed at example-creation time and notes drift risk.
- README.md points to integration docs while preserving MVP status accuracy.
- No provider-specific behavior is required for correctness.
...
```bash
git diff --check
test -f docs/integrations/README.md
test -d examples/mcp
test -d examples/workflows
```

## Task Prompt

Run TC27 only on branch `feature/terminal-commander-mvp`. Complete the objective above, stay inside the allowed files/areas, respect all forbidden files and invariants, verify the work, commit only verified changes, update this goal file's status fields, and report blockers instead of guessing.
...
Next goal:
- TC28 and TC28-load-performance-and-backpressure-tests.md
