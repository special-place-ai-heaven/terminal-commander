...
- Hardening tests and docs MUST label policy enforcement as advisory (daemon-side); kernel enforcement via Landlock/seccomp-bpf is roadmap, NOT MVP. Cite `docs/research/policy-prior-art.md`.

implementation_steps:
- Add tests for denied sensitive paths through daemon and MCP file tools.
- Add tests for `regex_size_limit` and `dfa_size_limit` handling on hostile patterns. Note: the Rust `regex` crate is linear-time and NOT vulnerable to catastrophic backtracking; the failure mode is resource limits, not exponential time. <<DECISION REQUIRED: concrete regex DoS test vectors targeting regex_size_limit / dfa_size_limit>>
- Add tests for long lines, binary-like input, invalid UTF-8 handling where applicable, and huge capture groups.
...
- `docs/security/HARDENING.md` MUST cite Landlock availability on WSL2 (kernel 5.15.57.1+, current WSL2 branch linux-msft-wsl-6.18.y) as the post-MVP enforcement path. <<DECISION REQUIRED: seccompiler Rust daemon integration patterns>>
...
- Any new dependency introduced by hardening tests must pass `cargo deny check` with the existing license allowlist (Category-A + CC0-1.0 for `notify`).
...
cargo nextest run --workspace -E 'test(security)'
cargo deny check
```
