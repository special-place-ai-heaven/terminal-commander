...
- docs/release/**
- README.md
- LICENSE (read-only verification; LICENSE is created by TC01a and required by TC04)
- NOTICE (read-only verification; NOTICE is created by TC01a)
- crates/*/src/** (per-file SPDX header audit sweep only; no logic changes)
...
- RELEASE.md MUST state: rmcp pin `=1.7.0`, MSRV 1.92, Edition 2024, MCP spec revision 2025-11-25, target platforms Linux + WSL2.
...
- Verify LICENSE at repo root contains the full Apache-2.0 text (SHA256 against the ASF official file). Verify NOTICE exists with rmcp Apache-2.0 relicensing transition note. Run `cargo deny check licenses` with the locked allowlist (Category-A + CC0-1.0).
- NOTICE or RELEASE.md notes the rmcp Apache-2.0 relicensing transition; cargo-deny allowlist covers both `MIT OR Apache-2.0` (legacy) and `Apache-2.0` (current).
...
- Install verification documents WSL systemd opt-in prerequisite (WSL 0.67.6+ and `/etc/wsl.conf` `[boot] systemd=true`) as the prerequisite for the optional systemd user unit.
