...
- Tools use `rmcp = "=1.7.0"` + MCP spec rev 2025-11-25.
- `file_watch` inherits TC18's 9P detection via daemon; MCP response includes `transport_mode` field so callers see when polling is in use.
...
- Denial responses MUST label enforcement as advisory (daemon-side); MCP responses MUST NOT imply kernel-level guarantee.
