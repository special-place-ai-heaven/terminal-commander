...
- Policy enforcement is ADVISORY in MVP; Landlock (kernel 5.13+, WSL2 5.15.57.1+) and seccomp-bpf are documented roadmap items, not MVP behavior. See docs/research/policy-prior-art.md.
- cap-std Dir handles are the MVP enforcement primitive for filesystem access scoping (in-process, not kernel-level).
- Default-deny sensitive path list (SSH keys, /etc/shadow, .pgpass, ~/.aws/credentials, ~/.config/gcloud, ~/.kube/config, token caches) anchored at README.md:294-297.
