...
Use this file directly with `/goal`:
    ...
    /goal .agent/goals/terminal-commander-mvp/TC30-end-to-end-mvp-demo-scenarios.md
...
objective:
- Create and verify end-to-end MVP demo workflows showing command execution, realtime bucket waiting, dynamic registry rule creation, file watching, and bounded context retrieval.

non_goals:
- Do not create polished release packaging.
- Do not require network access or root access for demos.
- Do not hide partial or blocked demo steps.

allowed_files_or_area:
- .agent/goals/terminal-commander-mvp/TC30-end-to-end-mvp-demo-scenarios.md
- examples/workflows/**
- tests/e2e/**
- scripts/demo/**
- docs/demo/**
...
- Each demo must prove pure-signal behavior: noisy source input produces small structured bucket output.
- Each demo must include expected commands, expected signal events, and expected evidence output.
- Demos must identify live, partial, blocked, or unverified steps explicitly.
- Demo scenarios MUST exercise only tools from the canonical 20 MCP tools (README.md:243-266); any tool the daemon does not implement live MUST be labeled blocked, not invented.
...
- Create a command-demo scenario using a safe noisy command or fixture-emitting script and bucket_wait.
- Create a registry-demo scenario where a missing rule is created, tested, activated, and matched against a stream.
- Create a file-watch-demo scenario using a temporary file and appended matching lines.
- file-watch demo MUST run on a native Linux path (e.g., `~/...`) by default; if `/mnt/c/...` is unavoidable, document expected PollWatcher latency.
- Create an event-context-demo scenario retrieving bounded context around a detected error.
- Add e2e tests or script-based demos that can run locally without root/network when possible.

acceptance_criteria:
- At least three end-to-end scenarios exist: command, dynamic registry, and file watch.
- At least one scenario retrieves event context with explicit before/after bounds.
- Demo outputs show small signal responses despite noisy input.
- Scripts or tests avoid root, network, secrets, and destructive commands.
- README.md accurately links to demos without overstating beta readiness.
- Probe types and sifter types referenced in demos must match the canonical 6-probe / 11-sifter lists (README.md:124-148); deviations MUST be labeled deferred.
...
cargo nextest run --workspace -E 'test(e2e)'
bash scripts/demo/run-all.sh
```

## Task Prompt

Run TC30 only on branch `feature/terminal-commander-mvp`. Complete the objective above, stay inside the allowed files/areas, respect all forbidden files and invariants, verify the work, commit only verified changes, update this goal file's status fields, and report blockers instead of guessing.
...
Next goal:
- TC31 and TC31-beta-packaging-and-release-checklist.md
