...
Use this file directly with `/goal`:
    ...
    /goal .agent/goals/terminal-commander-mvp/TC19-terminal-pty-probe-and-prompt-detection.md
...
objective:
- Add PTY/terminal-style command support with ANSI/carriage-return normalization, prompt detection, and bounded stdin writing for interactive workflows.

non_goals:
- Do not implement unrestricted shell automation or secret entry.
- Do not store sensitive prompt responses in clear event summaries.
- Do not add MCP stdin tools until policy and local API surfaces are ready.

allowed_files_or_area:
- .agent/goals/terminal-commander-mvp/TC19-terminal-pty-probe-and-prompt-detection.md
...
- tests/fixtures/terminal-streams/**
- docs/probes/**
...
- PTY frames must be normalized for ANSI escape sequences, carriage returns, progress updates, and partial lines.
- Prompt detection must emit structured events for interactive, sudo, or password-like waits without collecting secrets.
- stdin write support must be bounded and tied to a job/probe handle.
- PTY-spawned processes participate in the same process-group + SIGTERM-then-SIGKILL cleanup ladder as TC15 non-interactive jobs.
- Prompt detection is implemented as sifter rules from the canonical 11-set; reuse TC14 rule-pack format if practical.
- Password/secret prompt events MUST NOT include the user's response text or any echoed character payload; only the prompt itself + a redaction marker.
- Any new Cargo.toml created by this goal must set `license.workspace = true` (SPDX `Apache-2.0`).
- <<DECISION REQUIRED: ANSI normalization crate (vte vs strip-ansi-escapes)>>
- <<DECISION REQUIRED: pty-process tokio version compatibility check>>
...
- Use `pty-process` 0.5.3 with `async` feature (POSIX-only Linux + WSL2). `portable-pty` deferred to post-MVP Windows-native work. Document limitations in `docs/probes/TERMINAL_PTY.md`.
- Implement terminal normalization for ANSI stripping, carriage-return progress, partial lines, and prompt-like output.
- Implement prompt detector rules for generic input prompts, sudo prompts, and password prompts with safe summaries.
- Add bounded stdin write support at probe level if feasible, with tests using non-secret interactive commands.
- Document PTY limitations in docs/probes/TERMINAL_PTY.md.

acceptance_criteria:
- PTY tests demonstrate normalized output without progress spam flooding events.
- Prompt detector emits a structured signal event when a safe fixture or command waits for input.
- Password-like prompts are detected but no secret input is stored in event summaries or logs.
- stdin writing is implemented and tested or explicitly deferred with a stop/blocker note.
- No unrestricted interactive root shell behavior is added.
...
cargo clippy -p terminal-commander-sifters --all-targets -- -D warnings
cargo nextest run -p terminal-commander-probes
cargo nextest run -p terminal-commander-sifters
```

## Task Prompt

Run TC19 only on branch `feature/terminal-commander-mvp`. Complete the objective above, stay inside the allowed files/areas, respect all forbidden files and invariants, verify the work, commit only verified changes, update this goal file's status fields, and report blockers instead of guessing.
...
Next goal:
- TC20 and TC20-directory-and-artifact-probes.md
