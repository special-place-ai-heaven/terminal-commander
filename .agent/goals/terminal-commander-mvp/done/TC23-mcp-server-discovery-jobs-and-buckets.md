...
- MCP tools are exposed via `rmcp = "=1.7.0"` against MCP spec revision 2025-11-25. Cite `docs/research/mcp-rust-sdk.md`.
- bucket_wait MCP tool wraps TC17 contract verbatim - heartbeat=true + events=[] + next_cursor=<current bucket cursor> on timeout.
- Tool documentation MUST label policy denials as ADVISORY (daemon-side, not kernel-enforced) per TC22 framing.
...
```bash
cargo fmt --check
cargo clippy -p terminal-commander-mcp --all-targets -- -D warnings
cargo nextest run -p terminal-commander-mcp
```
