...
# TC17 - Realtime Bucket Waiter

Use this file directly with `/goal`:

    /goal .agent/goals/terminal-commander-mvp/TC17-realtime-bucket-waiter.md
...
- KEYSTONE NOTE: TC17 is the keystone primitive that validates the wait-for-signal-not-poll thesis at the heart of the product (`README.md:268-281`, `docs/research/_R2-delta-summary.md` F5). Every downstream MCP `bucket_wait` exposure and provider integration assumes this primitive is correct, prompt, and free of busy polling. Treat correctness over performance.

## Mini-Spec

objective:
- Implement bucket_wait semantics so clients can block for matching signal events by cursor, severity, kind filter, and timeout without polling raw output.

non_goals:
- Do not implement MCP bucket_wait tool yet unless only as a local API test seam.
- Do not use busy waiting or periodic tail reads as the implementation.
- Do not return raw stream frames from bucket_wait.

allowed_files_or_area:
- .agent/goals/terminal-commander-mvp/TC17-realtime-bucket-waiter.md
...
- bucket_wait accepts bucket_id, cursor, severity_min, optional event kinds, limit, and timeout_ms.
- If matching events arrive, return events and next_cursor immediately or promptly.
- On timeout (no matching event before timeout_ms) return `heartbeat=true`, `events=[]`, and `next_cursor=<current bucket cursor>` (the bucket's latest cursor at timeout instant; NOT a passthrough of the client's request cursor). Clients can detect liveness from cursor advancement on heartbeat as well as on event delivery.
- Wakeup is signal-driven via tokio `broadcast` or `Notify` registered against the BucketManager append path. Sleep-poll loops, periodic tail reads, and timer-based polling of the store are explicitly forbidden as implementation strategies (timeout fires via a single `tokio::time::sleep` or `tokio::select!` deadline).
- No raw frames or unbounded output may be returned from bucket_wait; the response shape is strictly bounded by `limit` and the canonical SignalEvent schema.
...
- Define BucketWaitRequest and BucketWaitResponse contracts if not already defined.
- Implement waiter registration and notification when events are appended to buckets.
- Support timeout and heartbeat behavior without busy polling.
- Support filtering by severity_min and optional event kinds.
- Add async or concurrency tests for wait returns on event, timeout heartbeat, multiple waiters, and cursor advancement.

acceptance_criteria:
- bucket_wait returns promptly when a matching event is appended.
- bucket_wait times out with heartbeat when no matching event arrives.
- Low-severity events do not wake a high-severity wait unless explicitly requested.
- Multiple concurrent waiters are supported or documented with tested limitations.
- No raw frames or unbounded output are returned.
...
Run TC17 only on branch `feature/terminal-commander-mvp`. Complete the objective above, stay inside the allowed files/areas, respect all forbidden files and invariants, verify the work, commit only verified changes, update this goal file's status fields, and report blockers instead of guessing.
...
Next goal:
- TC18 and TC18-file-probe-follow-create-and-rotate.md
