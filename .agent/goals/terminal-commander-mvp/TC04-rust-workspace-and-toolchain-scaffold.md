...
- Workspace crates must use stable names: terminal-commander-core, terminal-commander-sifters, terminal-commander-probes, terminal-commander-store, terminal-commanderd, terminal-commander-mcp, terminal-commander-cli. The 7-crate list is locked; `terminal-commander-store` is required (TC12/TC13 depend on it). README crate count is reconciled to 7 by TC01a (blocking dependency).
...
- Pinned toolchain: edition 2024, resolver "3", rust-toolchain 1.92.
- Workspace fields required: `[workspace.package]`, `[workspace.dependencies]`, `[workspace.lints]` (Cargo 1.74+ feature).
- Each per-crate Cargo.toml must include SPDX header: `license = "Apache-2.0"` (or `license.workspace = true` once workspace.package.license is set).
- cargo-deny license allowlist must include CC0-1.0 (required for `notify` transitive dep) in addition to standard Category-A licenses.
...
cargo deny check licenses || true # cargo-deny config may not exist yet; advisory at TC04
```
