...
- Include registry FTS5 search latency under load as a measured dimension.
...
- MVP load tests establish the rusqlite WAL throughput baseline. No prior measurement exists per `docs/research/_R1-beta-summary.md`. Tests record measured numbers as the baseline rather than gating on a target.
...
- Load tests record watcher transport (inotify vs PollWatcher) and target FS type so results are comparable across runs.
...
cargo nextest run --workspace -E 'test(load)' --no-fail-fast
```
